...
class ChatSerializer(serializers.ModelSerializer):
    first_user = serializers.SlugRelatedField(queryset=User.objects.all(), slug_field="username")
    second_user = serializers.SlugRelatedField(queryset=User.objects.all(), slug_field="username")
    ...
    def validate(self, data):
        ...
        if first_user == second_user:
            raise serializers.ValidationError("First user and second user must be different")
